...
export default function AdminPage() {
  ...
  const { data, isLoading, refetch } = useAdminTodos(filter)
  ...
  const getFilteredTodos = () => {
    ...
    if (filter.search) {
      ...
      todos = todos.filter(todo => {
        ...
        return todoText.includes(searchLower) || userName.includes(searchLower) || status.includes(searchLower)
      })
    }
    return todos
  }
  ...
  const paginatedTodos = filteredTodos.slice((filter.page - 1) * filter.limit, filter.page * filter.limit)
  ...
  return (
    <RoleGuard allowedRoles={['admin']} redirectTo="/todo">
      <div className="flex flex-col h-full bg-gray-50">
        {/* Page Title */}
        <section className="px-4 lg:px-8 py-4 lg:py-6 bg-white border-b border-gray-200">
            ...
            <form onSubmit={handleSearch} className="space-y-4 lg:space-y-0 lg:flex lg:items-center lg:gap-3 mb-6">
              ...
              <div className="flex flex-row gap-2 lg:gap-3">
                ...
                <button 
                  ...
                  disabled={isSpinning}
                  className="flex items-center justify-center bg-white border border-gray-300 text-sm font-medium px-4 py-2.5 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
